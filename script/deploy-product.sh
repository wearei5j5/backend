...
if [ -z "$IS_BLUE" ] && [ -z "$IS_GREEN" ]; then # blue, green 컨테이너 모두 실행중이 아닐 때
  ...
  docker-compose -f docker-compose.api.product.yml pull blue

  echo "2. blue container up"
  docker-compose -f docker-compose.api.product.yml up -d blue
...
if [ -z $IS_GREEN  ];then # IS_GREEN 변수가 비어있는 경우, Blue 컨테이너가 실행 중
  ...
  docker-compose -f docker-compose.api.product.yml pull green

  echo "2. green container up"
  docker-compose -f docker-compose.api.product.yml up -d green
  ...
  docker-compose -f docker-compose.api.product.yml stop blue
else
  echo "GREEN ==> BLUE"
  ...
  docker-compose -f docker-compose.api.product.yml stop green
fi